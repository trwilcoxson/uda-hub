...
class TicketResolution(Base):
    """Stores resolved ticket outcomes for long-term learning."""
    __tablename__ = 'ticket_resolutions'
    ticket_id = Column(String, ForeignKey('tickets.ticket_id'), primary_key=True)
    resolution_summary = Column(Text, nullable=False)
    resolution_agent = Column(String, nullable=False)
    resolution_type = Column(String, nullable=False)  # e.g., 'kb_article', 'action', 'escalation'
    articles_used = Column(Text)  # JSON list of article_ids used
    tools_used = Column(Text)  # JSON list of tool names used
    created_at = Column(DateTime, default=func.now())

    ticket = relationship("Ticket")

    def __repr__(self):
        return f"<TicketResolution(ticket_id='{self.ticket_id}', type='{self.resolution_type}')>"


class CustomerPreference(Base):
    """Stores customer preferences across sessions for personalized support."""
    ...
    __table_args__ = (
        UniqueConstraint('external_user_id', 'account_id', 'preference_key',
                         name='uq_customer_preference'),
    )

    def __repr__(self):
        return f"<CustomerPreference(user='{self.external_user_id}', key='{self.preference_key}')>"
